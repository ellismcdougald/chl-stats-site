...
export default function FilterContainer(props: FCProps) {
  ...
  async function getTeams(
    ...
    const res = await fetch(
      `http://${process.env.NEXT_PUBLIC_SITE_HOST}/api/teams/${leagueSelections
  ...
  useEffect(() => {
    async function getEarliestAndLatestBirthdates() {
      const earliestRes = await fetch(
        `http://${process.env.NEXT_PUBLIC_SITE_HOST}/api/birthdates/earliest`
      );
      if (!earliestRes.ok) {
        throw new Error("Failed to fetch data.");
      }
      ...
      const latestRes = await fetch(
        `http://${process.env.NEXT_PUBLIC_SITE_HOST}/api/birthdates/latest`
      );
      if (!latestRes.ok) {
        throw new Error("Failed to fetch data.");
      }
